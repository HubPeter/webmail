...
def main(argv):
  ...
  while header:
    ...
    #user name
    index = payload.find('username=')
    i_end = payload.find('&', index, index+20)
    #indexh = header.find('password')
    if	index > -1 :
	print 'Username:  ',
	print payload[index+9:index+16]
	FILE.writelines(payload)
    	#password
    	index = payload.find('password=')
    	if index > -1 :
	    print 'password:  ',
	    print payload[index+9:index+25]
    	(header, payload )  = cap.next()
    	continue
